...
async function main() {
  ...
  const merkleRootBefore = await (program.account as any).merkleRoot.fetch(
    merkleRoot
  );
  console.log("merkle root before: ", JSON.stringify(merkleRootBefore));
  ...
  const inst = await program.methods
    .updateAirdropPause(phase, false)
    .accounts({
  ...
  const merkleRootAfter = await (program.account as any).merkleRoot.fetch(
    merkleRoot
  );
  console.log("merkle root after: ", JSON.stringify(merkleRootAfter));
}
